...
CREATE INDEX IF NOT EXISTS idx_seats_room_status ON seats(room_id, status);
...
CREATE INDEX IF NOT EXISTS idx_res_seat_date ON reservations(seat_id, res_date);
CREATE INDEX IF NOT EXISTS idx_res_user_date ON reservations(user_id, res_date);
CREATE INDEX IF NOT EXISTS idx_res_status ON reservations(status);

-- 同一座位同一天,活跃预约时段不得重叠(数据库层兜底)
ALTER TABLE reservations DROP CONSTRAINT IF EXISTS excl_seat_overlap;
ALTER TABLE reservations ADD CONSTRAINT excl_seat_overlap
EXCLUDE USING gist (
    seat_id WITH =,
    res_date WITH =,
    tsrange((res_date + start_time)::timestamp, (res_date + end_time)::timestamp) WITH &&
) WHERE (status IN ('pending', 'checked_in', 'temp_leave'));

-- 同一用户同一天,活跃预约时段不得重叠(防止一人多占)
ALTER TABLE reservations DROP CONSTRAINT IF EXISTS excl_user_overlap;
ALTER TABLE reservations ADD CONSTRAINT excl_user_overlap
EXCLUDE USING gist (
    user_id WITH =,
    res_date WITH =,
    tsrange((res_date + start_time)::timestamp, (res_date + end_time)::timestamp) WITH &&
...
CREATE INDEX IF NOT EXISTS idx_credit_user ON credit_logs(user_id, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_wait_room_slot ON waitlist(room_id, res_date, status, created_at);
CREATE INDEX IF NOT EXISTS idx_wait_user ON waitlist(user_id);
...
CREATE INDEX IF NOT EXISTS idx_notify_user_unread ON notifications(user_id, is_read, created_at DESC);
